...
weights = {
    ...
    'wd1': tf.Variable(tf.random_normal([4160, 2048])),
...
def process(data):
    ...
    TestLabelCh511=np.zeros([int(size[1]/Win)-1,10])
    ...
    with tf.Session() as sess:
        # sess.run(init)
        # print('working')
        load_path = saver.restore(sess, "./Test_CNN_model_0413_2048_3_3_CH4_Anna_v1"
                                        ".ckpt")
        step = 1
        ...
        labelcount=np.bincount(pred_value)
        SZ = np.array(labelcount)
        ExpIndex=0;
        if np.max(labelcount)==2:
            for i in range(0, SZ.size, 1):
                if labelcount[i]==2:
                    ExpIndex = i
        elif np.max(labelcount)==1:
            ExpIndex = pred_value[1]
        elif np.max(labelcount) == 3:
            ExpIndex = pred_value[1]
