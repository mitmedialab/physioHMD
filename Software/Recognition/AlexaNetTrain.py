...
def norm(name, l_input, lsize=2):
    ...
    # return tf.nn.lrn(l_input, 1, bias=1.0, alpha=1.0, beta=0.5, name=name)
    return l_input

# 定义整个网络
def alex_net(_X, _weights, _biases, _dropout):
    # 向量转为矩阵
    _X = tf.reshape(_X, shape=[-1, 40, 50, 1])
...
weights = {
    ...
    'wd1': tf.Variable(tf.random_normal([1120, 2048])),
    'wd2': tf.Variable(tf.random_normal([2048, 2048])),
    'out': tf.Variable(tf.random_normal([2048, 12]))
}
biases = {
...
with tf.Session() as sess:
    ...
    while step  < training_iters:
        ...
        if step % display_step == 0:
            # 计算精度
            TrainAcc, TrainLoss= sess.run([accuracy,cost], feed_dict={x: trainData, y: LabelData, keep_prob: 1.})
            # 计算损失值
            TestAcc = sess.run([accuracy], feed_dict={x: TestData, y: TestLabel, keep_prob: 1.})
            print("TestAcc:",TestAcc)
            print("step " + str(step * batch_size) + ", Minibatch Loss= " + "{:.6f}".format(TrainLoss) + ", Training Accuracy = " + "{:.5f}".format(TrainAcc))
        step += 1
